...
- All your project code into a single main package (`news_now_BOT/`)
...
```bash
<root directory>
├── news_now_BOT/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `news_now_BOT`
...
2. Clone the `news_now_BOT` repository

3. Open a shell into your local `news_now_BOT` directory and run 
    ```bash
    ./rename-template.sh news_now_BOT
    ``` 
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `news_now_BOT`, in this example)
...
13. Put your main (resp. test) code in `news_now_BOT/` (resp. `test/`)

## How to do stuff

### Run your code as an application

This will execute the file `news_now_BOT/__main__.py`:
```bash
python -m news_now_BOT 
```
...
GitHub actions automatically release a new version of `news_now_BOT` on PyPi whenever commits are pushed on either the `main`/`master` or `develop` branches, as well as when new tags are pushed.
